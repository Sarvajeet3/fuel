...
class FuelView : public BView
{
	public:
	FuelView(BRect frame) :
		BView(frame, "fuelview", B_FOLLOW_ALL, B_WILL_DRAW)
	{
...
class FuelWindow : public BWindow
{
	BMessageRunner *runner;

public:
	FuelWindow(const char *title, int width, int height) :
				...
				0)
	{
		...
		FuelView* view = new FuelView(Bounds());
		AddChild(view);
...
class FuelApplication : public BApplication
{
public:
	FuelApplication() : BApplication("application/x-vnd.example.fuel")
	{
	}
	...
	void ReadyToRun() override {
		...
		FuelWindow *window = new FuelWindow(window_title, window_width, window_height);
		window->Show();
