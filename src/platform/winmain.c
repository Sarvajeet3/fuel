...
static BOOL InitApp(HINSTANCE hInstance, int nCmdShow)
{
	...
	/* Check files. */
	bFileOK = FALSE;
	if (FILE_EXISTS("main.fuel"))
		bFileOK = TRUE;
	else if (FILE_EXISTS("fuel.tank"))
		bFileOK = TRUE;
	if (!bFileOK)
	{
		log_error("No game data.");
		return FALSE;
	}
	...
	{
		log_info("Failed to initialize the graphics.");
		return FALSE;
	}
...
bool log_warn(const char *s, ...)
{
	...
	MessageBox(hWndMain, win32_utf8_to_utf16(buf), wszTitle, MB_OK | MB_ICONWARNING);
...
bool log_error(const char *s, ...)
{
	...
	MessageBox(hWndMain, win32_utf8_to_utf16(buf), wszTitle, MB_OK | MB_ICONERROR);
