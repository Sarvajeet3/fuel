 ...
 * Load a font file.
 */
bool load_font_file(int index, const char *fname)
{
	FT_Error err;
	int i;

	assert(index >= 0 && index < FONT_COUNT);

	/* Read a content of a font file. */
	if (!read_font_file_content(fname,
				    &font_file_content[index],
				    &font_file_size[index]))
		return false;

	/* Load a font file content. */
	err = FT_New_Memory_Face(library,
				 ...
				 &face[index]);
	if (err != 0) {
		log_error("Failed to load the font file \"%s\".", fname);
		return false;
	}

	/* Preload font glyphs. */
	for (i = 0; i < FONT_COUNT; i++) 
		get_glyph_width(i, 8, 'A');
				   ...
				   FT_Long *size)
{
	...
	/* Open a font file. */
	if (!open_rfile(file_name, &rf))
		return false;

	/* Get a size of the file. */
	if (!get_rfile_size(rf, &fsize)) {
		log_error("Failed to access the file \"%s\".", file_name);
		close_rfile(rf);
		return false;
	}
	*size = (FT_Long)fsize;
	...
	*content = malloc((size_t)*size);
	...
	if (!read_rfile(rf, *content, fsize, &actual)) {
 ...
 */
int get_glyph_width(int font_index, int font_size, uint32_t codepoint)
{
	int w, h;
 ...
 */
int get_glyph_height(int font_index, int font_size, uint32_t codepoint)
{
	int w, h;
