...
{
    @autoreleasepool {
        // For the package, find the bundle first.
        if (strcmp(fname, PACKAGE_FILE) == 0) {
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
                // Get the size of data01.arc.
                NSError *attributesError = nil;
                NSDictionary *fileAttributes = [[NSFileManager defaultManager] attributesOfItemAtPath:filePath error:&attributesError];
                NSNumber *fileSizeNumber = [fileAttributes objectForKey:NSFileSize];
                long long fileSize = [fileSizeNumber longLongValue];

                // If the package is not a dummy file, use it.
                if (fileSize > 100) {
        ...
        if (strncmp(fname, "mov/", 4) == 0) {
            NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
            NSString *filePath = [NSString stringWithFormat:@"%@/Contents/Resources/mov/%s", bundlePath, fname];
            if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
                const char *cstr = [filePath UTF8String];
                char *ret = strdup(cstr);
                if (ret == NULL) {
        ...
        // Other files. (including the package)
        NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
        NSString *basePath = [bundlePath stringByDeletingLastPathComponent];
        NSString *filePath;
        if (fname != NULL)
